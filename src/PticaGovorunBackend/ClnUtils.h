...
namespace wv
{
	...
	public:
		/*implicit*/ slice(std::vector<T> const& v)
			: data_(v.data()), dataEnd_(v.data() + v.size())
		{}

		slice(T const* start, T const* last)
			: data_(start), dataEnd_(last)
		{}
		...
		: data_(&*start._Unchecked()), dataEnd_(&*last._Unchecked())
			{}
		
		slice(std::initializer_list<T> const& l)
			: data_(std::begin(l)), dataEnd_(std::end(l))
		{}
	...
	>
	inline
	auto make_view(Array const& a)-> slice<typename Array::value_type>
		{
			return{ a };
		}

	template<class T>
	inline slice<T> make_view(T const* p, typename slice<T>::size_type const n)
	{
